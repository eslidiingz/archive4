...
function Category() {
  ...
  const handleChange = (e) => {
    ...
    // Case 2  : The user unchecks the box
      ...
      setFavList(favList.filter((item) => item !== value));
    }
  };
  ...
  return (
    <>
      <div>
        ...
        <div>
          {favList.map((item, i) => {
            return <div className='d-flex gap-2 flex-wrap my-3' key={i}>
              <h6 className='mb-0 bg-filler'> <i className="fal fa-times mgr-8 c-pointer"></i>{item}</h6>
            </div>
          })}
        </div>
      </div>
