            ...
            <div class="card p-4">
                <div class="d-flex justify-content-between align-items-center mb-3 flex-column flex-lg-row">
                    <div class="">
                        <h5 class="mb-0">
                            <b>
                                แก้ไขหน้าที่ของอาหารเสริมทางการแพทย์
                            </b>
                        </h5>
                    </div>
                </div>
                ...
                <form class="row" role="form" action="{{ url('dashboard/drug-target/edit') }}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <input type="hidden" name="id" value="{{ $drugTarget->id }}">
                    <div class="col-12 mb-3">
                        <label for="name">ชื่อหน้าที่ของอาหารเสริมทางการแพทย์</label>
                        <input type="text" class="form-control" name="name" id="name"  value="{{$drugTarget->name}}" placeholder="ชื่อหน้าที่ของอาหารเสริมทางการแพทย์" required>
                    </div>
                    <div class="col-12 text-center">
                        ...
                        <button type="button" class="btn btn-outline-pink"
                            data-toggle="modal" data-target="#editModal{{$drugTarget->id}}">
                            บันทึก
                        </button>
                    </div>

                    <div class="modal fade" id="editModal{{$drugTarget->id}}"
                        tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle">
                                        แก้ไขหน้าที่ของอาหารเสริมทางการแพทย์ #{{$drugTarget->name}}</h5>
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        คุณต้องการจะแก้ไขหน้าที่ของอาหารเสริมทางการแพทย์นี้ใช่หรือไม่?
                                    </div>
                                </div>
...
{{--
<form role="form" action="{{ url('dashboard/drug-target/update') }}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    <div class="col-md-12">
