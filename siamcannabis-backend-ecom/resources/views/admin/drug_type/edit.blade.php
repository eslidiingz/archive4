    ...
    <div class="row py-4">
        <div class="col-12">
            <div class="card p-4">
                ...
                <form class="row" role="form" action="{{ url('dashboard/drug-type/edit') }}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <input type="hidden" name="id" value="{{ $drugType->id }}">
                    <div class="col-12 mb-3">
                        <label for="name">ชื่อประเภทของผลิตภัณฑ์ยา</label>
                        <input type="text" class="form-control" name="name" id="name"  value="{{$drugType->name}}" placeholder="ชื่อหมวดหมู่" required>
                    </div>
                    <div class="col-12 text-center">
                        <a href="{{ url('dashboard/drug-type') }}"
                            class="btn btn-secondary">ย้อนกลับ</a>

                        <button type="button" class="btn btn-outline-pink"
                            data-toggle="modal" data-target="#editModal{{$drugType->id}}">
                            บันทึก
                        </button>
                    </div>

                    <div class="modal fade" id="editModal{{$drugType->id}}"
                        tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle">
                                        แก้ไขประเภทของผลิตภัณฑ์ยา #{{$drugType->name}}</h5>
...
{{--
<form role="form" action="{{ url('dashboard/drug-type/update') }}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    <div class="col-md-12">
