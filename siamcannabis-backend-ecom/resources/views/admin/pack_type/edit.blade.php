    ...
    <div class="row py-4">
        <div class="col-12">
            <div class="card p-4">
                ...
                <form class="row" role="form" action="{{ url('dashboard/pack-type/edit') }}" method="post" enctype="multipart/form-data">
                    {{csrf_field()}}
                    <input type="hidden" name="id" value="{{ $packType->id }}">
                    <div class="col-12 mb-3">
                        <label for="name">ชื่อประเภทการห่อ</label>
                        <input type="text" class="form-control" name="name" id="name"  value="{{$packType->name}}" placeholder="ชื่อประเภทการห่อ" required>
                    </div>
                    <div class="col-12 text-center">
                        <a href="{{ url('dashboard/pack-type') }}"
                            class="btn btn-secondary">ย้อนกลับ</a>

                        <button type="button" class="btn btn-outline-pink"
                            data-toggle="modal" data-target="#editModal{{$packType->id}}">
                            บันทึก
                        </button>
                    </div>

                    <div class="modal fade" id="editModal{{$packType->id}}"
                        tabindex="-1" role="dialog" aria-hidden="true">
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLongTitle">
                                        แก้ไขประเภทการห่อ #{{$packType->name}}</h5>
                                ...
                                <div class="modal-body">
                                    <div class="mb-3">
                                        คุณต้องการจะแก้ไขประเภทการห่อนี้ใช่หรือไม่?
                                    </div>
                                </div>
...
{{--
<form role="form" action="{{ url('dashboard/pack-type/update') }}" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
    <div class="col-md-12">
