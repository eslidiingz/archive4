<div>
    <div>
        ...
        <div class="row py-4 mx-0 mt-4" style="background-color: #fff; border-bottom: rgba(0, 0, 0, 0.16) 1px solid;">
            <div class="col-lg-6 col-md-6 col-12">
                <div class="row align-items-center">
                    <div class="col-lg-2 col-md-12 col-12 text-lg-left text-md-left text-left mt-lg-0 mt-md-4">
                        <h6 class="mb-0"><strong>{{ $shop->shops[0]->shop_name }} </strong></h6>
                    </div>
            ...
            {{ dd($invs) }}
            @if($shop->shop_id == $invsdata->shop_id)
            <div class="col-lg-6 col-md-6 col-12">
                <div class="row align-items-center">
                    <div
                    ...
                    <div class="col-lg-4 col-md-4 col-6">
                        <a href='#' style='color:#1947e3'>
                            @if($invsdata->status == 0)
                            อยู่ในตะกร้า
                            @endif
                            @if($invsdata->status == 1)
                            สั่งซื้อสินค้า
                            @endif
                            @if($invsdata->status == 2)
                            ชำระเงิน
                            @endif
                            @if($invsdata->status == 3)
                            จัดส่งสินค้า
                            @endif
                            @if($invsdata->status == 4)
                            ได้รับสินค้า
                            @endif
                            @if($invsdata->status == 5)
                            ยืนยันรับสินค้า
                            @endif
                            @if($invsdata->status == 6)
                            แจ้งยกเลิก
                            @endif
                            @if($invsdata->status == 99)
                            ยกเลิกสำเร็จ
                            @endif
                        </a>
                    </div>
        ...
        {{-- @foreach($invsdata->inv_products as $id) --}}
        <div class="row py-4 mx-0" style="background-color: #fff; border-bottom: rgba(0, 0, 0, 0.16) 1px solid;">
            <div class="col-lg-6 col-md-6 col-12">
                <div class="row">
                    <div class="col-12 mb-4 text-left">
                        <div class="media">
                            <img src="{{ asset('img/products/')  }}" style="width: 60px;" class="mr-3 rounded8px"
                                alt="...">
                            <div class="media-body">
                                <h6 class="mt-0"><strong>

                                        @foreach($product->whereIn('id',$invsdata->inv_products[$key]['product_id']) as $keyPro=>$pro)
                                        @if ( $pro->id == $invsdata->inv_products[$key]['product_id'] )
                                        {{ $pro->name}}
                                    </strong></h6>
                                <!-- have data -->
                                @if(isset($invsdata->inv_products[$key]['dis1']))
                                {{ $invsdata->inv_products[$key]['dis1']}}
                                @endif
                                <!-- not have data -->
                                @if(!isset($invsdata->inv_products[$key]['dis1']))
                                ...
                                <!-- have data -->
                                @if(isset($invsdata->inv_products[$key]['dis2']))
                                {{ $invsdata->inv_products[$key]['dis2']}}
                                @endif
                                <!-- not have data -->
                                @if(isset($invsdata->inv_products[$key]['dis2']))
                                <h6 style="color:gray">ไม่มีตัวเลือก</h6>
            ...
            <div class="col-lg-2 col-md-2 col-6">
                <div class="row">
                    <div class="col-12 mb-2 text-left">
                        <h5 class="mb-0" style='color:#c45e9f'><strong>฿ {{ $invsdata->inv_products[$key]['price'] }}</strong>
                        </h5>
                    </div>
            ...
            <div class="col-lg-2 col-md-2 col-4">
                <h5 class="mb-0" style='color:#c45e9f'><strong>฿ {{ $invsdata->inv_products[$key]['price'] }}</strong></h5>
            </div>
        </div>
